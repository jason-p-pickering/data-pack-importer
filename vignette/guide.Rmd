...
#Parse the PNSU data
d_obj<-DataPackR(disagg_tool, distribution_method = distribution_year, support_files_path = support_files)
```
...
loadSecrets(secrets)
d_obj<-DataPackR(disagg_tool,
               distribution_method = distribution_year,
               support_files_path = support_files)

d<-d_obj$data
names(d)<-c("dataElement","period","orgUnit","categoryOptionCombo","attributeOptionCombo","value")
...
Mechanisms are associated with specific periods and organisation units. We need to check to be sure that there are no invalid mechanisms in the data payload. Invoke the following command, which should return no rows if all mechanisms are valid. 
...
doMC::registerDoMC(cores=parallel::detectCores())# or however many cores you have access to
}

vr_violations<-validateData(data = d, return_violations_only = TRUE, parallel = TRUE)
...
vr_violations <- vr_violations %>% 
  tidyr::separate(formula, "<=",into=c("left","right")) %>%
  mutate(left = as.numeric(left),
         right = as.numeric(right)) %>%
  mutate(diff_pct = round(left / right,2),
         diff_abs = abs(left - right) )

output_file_path <- paste0(
  dirname(d_obj$wb_info$wb_path),
  "/validation_rules_",
  d_obj$wb_info$wb_type,
  "_",
  d_obj$wb_info$ou_name,
  "_",
  format(Sys.time(), "%Y%m%d%H%M%S"),
  ".csv"
)
write.csv(vr_violations,file=output_file_path,row.names = FALSE)
message(paste0("Saved validation rule violations to ",output_file_path))
```
